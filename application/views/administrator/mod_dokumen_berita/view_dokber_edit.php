...
<html>
  ...
  .sekolah:hover{
    ...
    echo "<div class='col-md-12'>
              ...
              echo form_open_multipart('administrator/edit_dokber',$attributes); 
              echo "<div class='col-md-12'>
                  <table class='table table-condensed table-bordered'>
                  <tbody>
                    <input type='hidden' name='id' value='$dokber[id]'>
                   <tr><th scope='row'>Kategori</th>               
                    <td><select name='c' class='form-control' required>
                      <option value='' selected>- Pilih Kategori -</option>";
                        foreach ($katber->result_array() as $row){
                          if ($dokber['id_katber'] == $row['id_kategori']){
                    ...
                    <tr><th width='120px' scope='row'>Judul Dokumen</th> 
                    <td><select name='b' class='form-control' required>
                      
                      <option value='' selected>- Pilih Judul -</option>";
                        foreach ($judul_berita2->result_array() as $row){
                          if ($dokber['idberita'] == $row['id_berita']){
                            echo "<option value='$row[id_berita]'  selected>$row[judul_berita]</option>";
                          }else{
                            echo "<option value='$row[id_berita]'>$row[judul_berita]</option>";
                          }
                        }
                    ...
                    <td><input type='file' class='form-control' name='d'>";
                      if ($dokber['dokumen'] != ''){ echo "<i style='color:red'>Lihat Dokumen Saat ini : </i><a target='_BLANK' href='".base_url()."asset/dokumen/$dokber[dokumen]'>$dokber[dokumen]</a>"; } echo "</td></tr>
                  </tbody>
                  </table>
                </div>
              </div>
            ...
            <div class='col-md-12'>
              ...
              echo "<div class='col-md-12'>
                  <table class='table table-condensed table-bordered'>
                  <tbody>
                    <input type='hidden' name='id' value='$dokber_en[id]'>
                   <tr><th scope='row'>Category</th>               
                    <td><select name='j' class='form-control' required>
                      <option value='' selected>- Choose Category -</option>";
                        foreach ($katber_en->result_array() as $row){
                          if ($dokber_en['id_katber'] == $row['id_kategori']){
                            echo "<option value='$row[id_kategori]' selected>$row[nama_kategori]</option>";
                          }else{
                            echo "<option value='$row[id_kategori]'>$row[nama_kategori]</option>";
                          }
                    ...
                    <tr><th width='120px' scope='row'>Document Title</th> 
                    <td><select name='k' class='form-control' required>
                      
                      <option value='' selected>- Choose Title -</option>";
                        foreach ($judul_berita2_en->result_array() as $row){
                          if ($dokber_en['idberita'] == $row['id_berita']){
                            echo "<option value='$row[id_berita]'  selected>$row[judul_berita]</option>";
                          }else{
                            echo "<option value='$row[id_berita]'>$row[judul_berita]</option>";
                          }
                    ...
                    <td><input type='file' class='form-control' name='l'>";
                      if ($dokber_en['dokumen'] != ''){ echo "<i style='color:red'>View Current Document : </i><a target='_BLANK' href='".base_url()."asset/dokumen/$dokber_en[dokumen]'>$dokber_en[dokumen]</a>"; } echo "</td></tr>
                  </tbody>
                  </table>
                </div>
              </div>
              ...
              <div class='box-footer'>
                    ...
                    <a href='../dokber'><button type='button' class='btn btn-default pull-right'>Cancel</button></a>
