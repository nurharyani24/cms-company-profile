          ...
          <div class="col-xs-12">  
              <div class="box">
                <div class="box-header">
                  <h3 class="box-title">Semua Dokumen Berita</h3>
                  <a class='pull-right btn btn-danger' href='<?php echo base_url(); ?>administrator/tambah_dokber'>Tambahkan Dokumen</a>
                </div><!-- /.box-header -->
                <div class="box-body">
                  ...
                  <?php 
                    $no = 1;
                    foreach ($dokber->result_array() as $row){
                    foreach ($jdl_berita->result_array() as $ber){
                    if($row['idberita'] == $ber['id_berita']){
                    echo "<tr><td>$no</td>
                          <td>$ber[judul_berita]</td>";
                          foreach($kat_berita->result_array() as $kat){
                            if($kat['id_kategori'] == $row['id_katber']){
                              echo "<td>$kat[nama_kategori]</td>";
                            }
                          }
                          echo "<td>$row[dokumen]</td>";
                          echo "<td><center>
                                <a class='btn btn-success btn-xs' title='Edit Data' href='".base_url()."administrator/edit_dokber/$row[id]'><span class='glyphicon glyphicon-edit'></span></a>
                                <a class='btn btn-danger btn-xs' title='Delete Data' href='".base_url()."administrator/delete_dokber/$row[id]' onclick=\"return confirm('Apa anda yakin untuk hapus Data ini?')\"><span class='glyphicon glyphicon-remove'></span></a>
                              </center></td>
                          </tr>";
                      $no++;
                    }
