		...
		*/
		function login()
		{
			if ($this->tank_auth->is_logged_in()) {									// logged in
				...
				$this->load->view('auth/logon', $data);
			}
		}
		...
		function lockprint()
		{
			...
			elseif ($this->tank_auth->is_logged_in(FALSE)) {						// logged in, not activated
				...
				// Get login for counting attempts to login
				if ($this->config->item('login_count_attempts', 'tank_auth') AND
				($login = $this->tank_auth->get_username())) {
					$login = $this->security->xss_clean($login);
					} else {
					$login = '';
				}
				...
				//if ($this->form_validation->run()) {								// validation ok
					...
					//$this->session->set_userdata(array(
						...
						//));
					// success
						redirect($this->session->userdata('url_print'));
					...
					$this->session->set_flashdata('error','Password Not Match, Please Try Again');
						redirect($this->session->userdata('url_redirect'));
					
					}
				//}
				//$data['show_captcha'] = FALSE;
				//if ($this->tank_auth->is_max_login_attempts_exceeded($login)) {
					//redirect('asset/t_header_mutasi');
					//$this->session->set_flashdata('error','Password Not Match, Please Try Again');
					//redirect('asset/t_header_mutasi');
				//}
				//$data['title']="LOCKED SCREEN";
				
				//$data['user_id']	= $this->tank_auth->get_user_id();
				//$data['username']	= $this->tank_auth->get_username();
				//$this->template->locked('locked', $data);
			}
		}
		...
		*/
		function register()
		{
			if ($this->tank_auth->is_logged_in()) {									// logged in
				...
				if ($use_username) {
					$this->form_validation->set_rules('username', 'Username', 'trim|required|min_length['.$this->config->item('username_min_length', 'tank_auth').']|max_length['.$this->config->item('username_max_length', 'tank_auth').']');
				}
