...
class Model_utama extends CI_model{
    ...
    function kategori($dari, $jumlah){
        return $this->db->query("SELECT * FROM kategori_berita where aktif='Y' ORDER BY id_kategori ASC LIMIT $dari, $jumlah");
    }

    function berita_perkategori($id, $dari, $jumlah){
        return $this->db->query("SELECT a.*, b.nama_kategori FROM berita a LEFT JOIN kategori_berita b ON a.id_kategori=b.id_kategori where a.id_kategori='$id' ORDER BY a.id_berita DESC LIMIT $dari, $jumlah");
    }
    ...
    function mainmenu(){
        return $this->db->query("SELECT * FROM main_menu where aktif='Y' ORDER BY id_main ASC");
    }

    function submenu($id){
        return $this->db->query("SELECT  * FROM sub_menu WHERE id_main='$id' AND aktif='Y' ORDER BY id_sub ASC");
    }

    function submenu1($id){
        return $this->db->query("SELECT * FROM sub_menu WHERE id_sub='$id' AND id_sub!='0' AND aktif='Y' ORDER BY id_sub ASC");
    }

    function subsubmenu($id){
        return $this->db->query("SELECT * FROM subsub_menu WHERE id_sub='$id' AND aktif='Y' ORDER BY id_subsub ASC");
    }

    function subsubmenu1($id){
        return $this->db->query("SELECT * FROM subsub_menu WHERE id_subsub='$id' AND id_subsub!='0' AND aktif='Y' ORDER BY id_subsub ASC");
    }
